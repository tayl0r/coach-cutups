...
final class CommentaryEventTests: XCTestCase {
    func test_allKindsRoundtripThroughJSON() throws {
        ...
        let events: [CommentaryEvent] = [
            .init(recordTime: 0.0, kind: .play(sourceTime: 0)),
            .init(recordTime: 1.5, kind: .pause(sourceTime: 1.5)),
            .init(recordTime: 2.0, kind: .play(sourceTime: 1.5)),
        ...
        XCTAssertEqual(decoded[0].kind, .play(sourceTime: 0))
        XCTAssertEqual(decoded[1].kind, .pause(sourceTime: 1.5))
        XCTAssertEqual(decoded[6].kind, .clearAll)
