...
- **Control socket adapter**: thin wrapper around the bus that exposes commands and a curated event subscription over TCP loopback (`127.0.0.1`, OS-chosen port) when `--control-socket=PORT` is passed. Gated behind a `control-socket` Cargo feature, **enabled by default for dev/test builds and disabled for release packaging** via `cargo build --release --no-default-features`. Release binaries shipped to end users do not contain the adapter code at all.
